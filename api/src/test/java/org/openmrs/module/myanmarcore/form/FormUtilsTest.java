...
package org.openmrs.module.myanmarcore.form;
...
public class FormUtilsTest extends BaseModuleContextSensitiveTest {
	...
	public void getFormXmlPath_shouldReturnXmlPathIfExists() {
		...
		resource.setValue("myanmarcore:test1.xml");
		Context.getFormService().saveFormResource(resource);

		Assert.assertEquals(new UiResource("myanmarcore", "test1.xml"), FormUtils.getFormXmlResource(form));
	}

	@Test
	public void setFormXmlPath_shouldSetPathAsFormResource() {
		Form form = Context.getFormService().getForm(1);
		FormUtils.setFormXmlResource(form, new UiResource("myanmarcore", "test2.xml"));

		FormResource resource = Context.getFormService().getFormResource(form, FormUtils.RESOURCE_HFE_XML_PATH);
		Assert.assertEquals("myanmarcore:test2.xml", resource.getValue());
	}

	@Test
	public void getHtmlForm_shouldCreateDynamicHtmlFormFormXmlPathResource() throws Exception {
		Form form = Context.getFormService().getForm(1);
		FormUtils.setFormXmlResource(form, new UiResource("myanmarcore", "test3.xml"));
		...
		when(resourceFactory.getResourceAsString("myanmarcore", "htmlforms/test3.xml")).thenReturn(xmlContent);
